...
The `:` character has two syntactic purposes in Julia. The first form creates a `Symbol`,
...
Every module](@ref modules) has its own [`eval` function that evaluates expressions in its global
...
objects: the `parse` function, which takes a string of Julia code and returns the corresponding
...
We have already seen a function `f(::Expr...) -> Expr` in a previous section. In fact, `macroexpand`
...
the returned expression as an argument to the `string` call; see [`error.jl`](https://github.com/JuliaLang/julia/blob/master/base/error.jl)
