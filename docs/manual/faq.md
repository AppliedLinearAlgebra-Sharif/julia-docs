...
کد بالا، با توجه به مقدار آرگومان‌های ورودی، خروجی‌اش از جنس `Int`
یا
`Float64`
می‌باشد.
...
به طور مشابه نمی‌توانید بر روی پارامترهای نوع، محاسبات (شامل عملیات‌های ساده مثل جمع یا تفریق) کنید. در عوض، این نوع محدودیت‌ها و روابط را می‌توانید از طریق پارامترهای نوع دیگری که در سازنده آورده می‌شوند، اعمال کنید.
...
در این مثال کاربر می‌خواهد همواره پارامتر سوم یک واحد بیشتر از پارامتر دوم باشد. این کار را می‌توان با استفاده از یک پارامتر نوع که در
[سازنده درونی](@ref man-inner-constructor-methods)
 چک می‌شود، اعمال کرد (می‌توانید آن را با شروط دیگر نیز ترکیب کنید):
...
این کار معمولا هزینه‌ای ندارد زیرا کامپایلر می‌تواند تست‌های معتبر بودن نوع‌های غیر انتزاعی را ترکیب کند. اگر چندتا از پارامترها نیاز به محاسبه داشته باشند، بهتر است تا از یک
[سازنده‌ی خارجی](@ref man-outer-constructor-methods)
 برای این محاسبات استفاده کنید:
...
در نگاه اول، این روش نسبتا منطقی به نظر می‌آید، زیرا
9223372036854775807
بسیار به
 9223372036854775808
 نسبت به
 9223372036854775808-
نزدیک‌تر است؛ و اعداد صحیح همچنان به روشی طبیعی با اندازه ثابت نمایش داده می‌شوند که با C و Fortran تطابق دارد. اما، محاسبات اشباعی اعداد صحیح، مشکلات عمیقی دارند.
...
`everywhere module Foo@`
 روی همه‌ی نودها، `Foo` را تعریف می‌کند. با این وجود، صدا کردن
 `()Foo.foo`
  یک binding سراسری `gvar` روی نود محلی می‌سازد، ولی این در نود ۲ پیدا نشد و باعث یک خطای `UndefVarError` شد.
...
۱. flag را در `julia` برابر با
 `compiled-modules=no--`
قرار دهید
تا پیش-کامپایل کردن را خاموش کنید.

۲. با استفاده از `pushfirst!(DEPOT_PATH, private_path)`  یک منبع (depot) قابل نوشتن خصوصی ایجاد کنید، به طوری که `private_path` یک مسیر یکتا به این فرآیند `julia` است. این می‌تواند از طریق عوض کردن متغیر محیطی  `JULIA_DEPOT_PATH` به
...
لینک‌های دانلود انواع مختلف را می‌توانید از صفحه‌ی دانلود، در
...
[محیط](https://docs.julialang.org/en/v1/manual/code-loading/#Environments-1)
 پیش‌فرض خودش را دارد. در نتیجه، هنگامی که یک نسخه‌ی جزئی جولیا را نصب کنید، بسته‌هایی که به نسخه‌هایی جزئی قبلی جولیا اضافه کرده‌اید، به طور پیش‌فرض در دسترس نخواهند بود. محیط (environment) هر نسخه‌ی جولیا در پرونده‌های `Project.toml` و `Manifest.toml` در یک پوشه در
 `/julia/environments.`
  ذخیره می‌شوند که با شماره‌ی جولیا تطابق دارد، به عنوان مثال
